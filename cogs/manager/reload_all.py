...
class CogStuff(commands.Cog):
    ...
    async def reload(self, ctx: Context, what: str, name: str):
        ...
        try:
            await self.bot.reload_extension(f"cogs.{what}.{name}")
            await ctx.reply(f"Reloaded `cogs.{what}.{name}`")
        except Exception as e:
            await ctx.reply(f"`{e}`")

    @commands.guild_only()
    @commands.hybrid_command(name="load", description="And now I'll wave, so long! -- Placeholder")
    async def load(self, ctx: Context, what: str, name: str):
        if SemiFunc.command_disabled(ctx):
            await ctx.reply("That command is currently disabled.")
            return
        ...
        try:
            await self.bot.load_extension(f"cogs.{what}.{name}")
            await ctx.reply(f"Loaded `cogs.{what}.{name}`")
        except Exception as e:
            await ctx.reply(f"`{e}`")

    @commands.guild_only()
    @commands.hybrid_command(name="unload", description="And now I'll wave, so long! -- Placeholder")
    async def unload(self, ctx: Context, what: str, name: str):
        if SemiFunc.command_disabled(ctx):
            await ctx.reply("That command is currently disabled.")
            return
        ...
        try:
            await self.bot.unload_extension(f"cogs.{what}.{name}")
            await ctx.reply(f"Unloaded `cogs.{what}.{name}`")
        except Exception as e:
            await ctx.reply(f"`{e}`")
    ...
    async def reloadall(self, ctx: Context):
            ...
            if what == "__pycache__":
                ...
                if what.endswith(".py"):
                    ...
                    try:
                        ...
                        failed.append(f"listeners.{name}")
            ...
            if gud:
                for file in os.listdir(f"listeners/{what}"):
                    ...
                    try:
                        ...
                        failed.append(f"listeners.{what}.{name}")
        ...
        await ctx.send(f"Reloaded these successfully:\n```{successA}```")
        await ctx.send(f"Unable to reload these successfully:\n{failedA}")
