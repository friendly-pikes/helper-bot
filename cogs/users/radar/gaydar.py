...
class UserCommands__Radar__Gaydar(commands.Cog):
    ...
    # @commands.guild_only()
    @commands.hybrid_command(name="gaydar", aliases=["howgay"])
    @app_commands.allowed_installs(guilds=True, users=True)
    @app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def gaydar(self, ctx: Union[Context, commands.context.Context], user: Union[discord.Member, discord.User]):
        """
        ...
        embed = await SemiFunc.pikesRadar(self.bot, user, "gay")
...
async def setup(bot):
    await bot.add_cog(UserCommands__Radar__Gaydar(bot))
